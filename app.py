...
# --- 5. 側邊欄導覽 ---
st.sidebar.markdown("### 🏫 班級管理系統")
role = st.sidebar.radio("身分切換：", ["📝 學生：成績錄入", "📊 老師：數據與診斷"])

# --- 6. 學生錄入介面 (保留所有即時更新與刪除功能) ---
if role == "📝 學生：成績錄入":
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 提交成績"):
            ...
            # 即時寫入 Session State 與 Google Sheets
    ...
    if not my_records.empty:
        ...
        if st.button("🗑️ 撤回最後一筆錄入資料"):
            ...
            if not idx.empty:
                ...
                conn.update(spreadsheet=url, worksheet="成績資料", data=st.session_state['df_grades'])
                st.warning("資料已撤回並從資料庫刪除！"); time.sleep(0.5); st.rerun()
...
# --- 7. 老師專區 (連動優化) ---
    ...
    if st.session_state['authenticated']:
        ...
        with tabs[0]:
            ...
            with c_d3: mode = st.radio("功能：", ["個人段考成績單", "班級段考總表"], horizontal=True)
        ...
        with tabs[1]:
            ...
            if st.session_state['current_stu'] and st.session_state['current_analysis_data'] is not None:
                ...
                if st.button("🚀 生成診斷報告 (根據當前儀表板搜尋結果)", use_container_width=True):
                    genai.configure(api_key=st.secrets["gemini"]["api_key"])
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    data_str = st.session_state['current_analysis_data'][["科目", "分數", "等級", "班平均"]].to_string(index=False)
                    with st.spinner("AI 分析中..."):
                        prompt = f"你是台灣國中班導師，請診斷學生{st.session_state['current_stu']}的{st.session_state['current_exam']}數據：\n{data_str}\n請給予表格、優勢分析、待改進處與導師鼓勵。"
                ...
                st.warning("請先在『成績儀表板』分頁選好學生後，再來這裡進行診斷。")
