...
st.markdown("""
    ...
    .report-card {
        background: white; padding: 25px; border-radius: 15px; border: 2px solid #333; color: #333; line-height: 1.6;
    }
    </style>
...
if role == "📝 學生：成績錄入":
    ...
    if not my_records.empty:
        ...
        if st.button("🗑️ 撤回最後一筆"):
            # 找到該學生最後一筆資料的索引並刪除
            target_index = st.session_state['df_grades'][st.session_state['df_grades']["姓名"] == name].index[-1]
            st.session_state['df_grades'] = st.session_state['df_grades'].drop(target_index).reset_index(drop=True)
            conn.update(spreadsheet=url, worksheet="成績資料", data=st.session_state['df_grades'])
            st.warning("已成功撤回最後一筆錄入！"); time.sleep(0.5); st.rerun()
    else:
        st.info("目前尚無您的錄入紀錄")
        ...
        with tabs[0]:
                    ...
                    for sub in SUBJECT_ORDER:
                        ...
                        if not match.empty:
                            ...
                            res = {
                                ...
                                "班平均": format_num(sub_all.mean()), 
                                "標準差": round(sub_all.std(), 2) if len(sub_all) > 1 else 0,
                                "中位數": sub_all.median()
                            }
                        ...
                        if sub == "公民":
                            ...
                            if not soc_data.empty:
                                ...
                                sr = {
                                    "科目": "★社會(整合)", "分數": round(sa, 2), "等級": sg, "點數": sp, 
                                    "班平均": format_num(soc_avg_pool["分數"].mean()),
                                    "標準差": round(soc_avg_pool["分數"].std(), 2) if len(soc_avg_pool) > 1 else 0
                                }
            ...
            # B. 班級總表
                ...
                if not tdf.empty:
                    ...
                    sub_cols = [s for s in SUBJECT_ORDER if s in piv.columns]
                    piv["總成績"] = piv[sub_cols].sum(axis=1)
            ...
            # C. 平時成績
                ...
                st.session_state['ai_sync_data'] = {"title": f"{t_s} 平時紀錄", "content": p_df.to_string()}

        # --- 8. AI 智慧診斷 (極簡版 UI) ---
        with tabs[1]:
            st.header("AI 智慧診斷") 
            if st.session_state['ai_sync_data'].get("title"):
                st.write(f"分析目標：{st.session_state['ai_sync_data']['title']}")
                if st.button("根據當前搜尋結果生成深度診斷"):
                    genai.configure(api_key=st.secrets["gemini"]["api_key"])
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    with st.spinner("正在進行深度學力診斷中..."):
                        prompt = f"""
                        你現在是一位專業的國中導師，請針對以下「{st.session_state['ai_sync_data']['title']}」的數據進行深度的學力診斷。
                        必須包含：
                        1. 根據班平均與分布，指出落點相對位置。
                        2. 根據標準差分析競爭力（標準差小表精準度競賽，大表程度落差大）。
                        3. 診斷科目均衡度。
                        4. 提供 3 個具體行動建議。
                        數據內容：
