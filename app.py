...
st.markdown("""
    ...
    .report-card code, .report-card pre { background-color: transparent !important; color: inherit !important; font-family: inherit !important; padding: 0 !important; }
...
if role == "📝 學生：成績錄入":
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 ✅ 提交成績"):
            ...
            new_row = pd.DataFrame([{"時間戳記": now_tw, "學號": sid, "姓名": name, "科目": subject, "分數": int(score), "考試類別": etype, "考試範圍": exam_range}])
...
# --- 7. 老師專區 (強化 AI 與 報表功能) ---
    ...
    if st.session_state['authenticated']:
        ...
        with tabs[0]: # 數據查詢中心
            ...
            if mode == "個人段考成績單":
                ...
                if not p_pool.empty:
                    ...
                    st.session_state['current_rpt_df'] = pd.DataFrame(rows) # 更新報表快照
                    st.session_state['current_rpt_name'] = f"{t_s}_{t_e}_成績單"
                    ...
                    st.dataframe(st.session_state['current_rpt_df'], hide_index=True, use_container_width=True)
                ...
                if not tdf.empty:
                    ...
                    st.session_state['current_rpt_df'] = piv.reset_index() # 更新報表快照
        ...
        with tabs[1]: # 🤖 AI 智慧診斷 (強化解釋)
            st.subheader("🤖 AI 智慧數據解析")
            ...
            if st.button("🚀 生成分析報告"):
                ...
                f_cat = "平時考" if "平時" in ai_type else "第一次段考"
                t_data = f_df[f_df["考試類別"] == f_cat]
                s_data = t_data[t_data["姓名"] == ai_name]
                if not s_data.empty:
                    stats = []
                    for s in s_data['科目'].unique():
                        s_avg = s_data[s_data['科目']==s]['分數'].mean()
                        c_avg = t_data[t_data['科目']==s]['分數'].mean()
                        c_std = t_data[t_data['科目']==s]['分數'].std()
                        stats.append(f"- {s}: 個人={format_num(s_avg)}, 班均={format_num(c_avg)}, 標準差(σ)={format_num(c_std)}")
                    
                    prompt = f"""
                    你是一位專業的國中班導師，請根據以下數據進行診斷：
                    學生姓名：{ai_name}
                    考試類別：{f_cat}
                    數據清單：
                    {stats}
                    ...
                    請依據以下重點撰寫建議：
                    1. 解釋「標準差(σ)」：若標準差大，說明班級此科目表現兩極化，請評估學生在其中的相對競爭力；若標準差小，說明全班程度接近。
                    2. 分析優勢與劣勢學科。
                    3. 給予具體的學習改進建議與鼓勵。
                    請使用親切但專業的語氣。
                    """
                    with st.spinner("AI 正在深度解析班級數據..."):
                        res = model.generate_content(prompt)
                        st.markdown(f'<div class="report-card">{res.text}</div>', unsafe_allow_html=True)

        with tabs[2]: # 📥 報表輸出中心 (完整恢復)
            st.subheader("📥 數據報表下載")
            if st.session_state['current_rpt_df'] is not None:
                st.info(f"📋 目前快照資料：{st.session_state['current_rpt_name']}")
                ...
                st.download_button(
                    label="💾 下載當前報表 (CSV 檔案)",
                    data=csv,
                    file_name=f"{st.session_state['current_rpt_name']}.csv",
                    mime="text/csv",
                    use_container_width=True
                )
            else:
                st.warning("請先在『數據查詢中心』進行查詢，報表將會自動出現在此處供下載。")
